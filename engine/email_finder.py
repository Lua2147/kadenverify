...
Waterfall:
  ...
  4. Enrichment fallback for catch-all / inconclusive domains
"""
...
async def find_email(
    ...
    # --- Phase 4: Catch-all / inconclusive enrichment ---

    # 4a: Apollo local DB (zero cost)
    apollo_local = _lookup_apollo_local(first_name, last_name, domain)
    if apollo_local:
        apollo_local_candidate = apollo_local
        candidates.append(apollo_local_candidate)
        return FinderResult(
            email=apollo_local_candidate.email,
            confidence=apollo_local_candidate.confidence,
    ...
    # 4d: Pattern scoring fallback (catch-all domains)
